...
#인덱싱 : 인덱스 이용한 요소 추출  '유재석'[0] , '강호동'[1] , '신동엽'[2] ,'서장훈'[3]
        # 유재석[-4] , 강호동[-3] , 신동엽[-3] , 서장훈[-1]
print(studentList) #전체출력
print(studentList[0])   #유재석
print(studentList[1])   #강호동
print(studentList[2])   #신동엽
print(studentList[3])   #서장훈
# print(studentList[4])   #오류 인덱스 범위 벗어남.
print(studentList[-1])  #서장훈
print(studentList[-2])  #신동엽
print(studentList[-3])  #강호동

#슬라이싱 : 인덱싱 를 이용한 요소들 추출 , [시작인덱스[포함] : 끝인덱스[포함x] : 증감단위 ]
...
print(studentList[2:])          #['신동엽' ,'서장훈'] , 생략시 마지막 인덱스 까지
...
del b[1:]       #슬라이싱 활용한 요소들 삭제, 1, 인덱스부터 마지막 인덱스까지 삭제
print(b)
...
#.insert( 인덱스 , 요소값): 리스트의 특정 인덱스에 요소값 추가
a.insert(0,6)   #0번인덱스의 6삽입 , 삽입된 위치 뒤로 한칸씩 밀림
print(a)     #[6, 1, 2, 3, 4]

#3. .remove(삭제할 요소값): 리스트의 삭제할 요소값이 존재하면 삭제
a.remove(1)
print(a)            #[6, 2, 3, 4]
# a.remove(1)         #존재하지 않는 값이면 예외발생
# print(a)
...
#5. .count(요소값)     : 리스트의 해당 찾을 요소값이 존재하는 개수 반환
a=[1,2,3,1]
print(a.count(1))       # 2, 리스트내 1의 요소가 2개

#6. .index(찾을 요소값)      : 리스트의 해당 찾을요소값이 존재하는 인덱스 반환
print(a.index(2))
    #1, 리스트내 2의 유치는 1 반환

#7. .extend(리스트)        : 리스트의 해당 리스트를 더해준다.
b= [4,5]
a.extend(b)
print(a)                #[1, 2, 3, 1, 4, 5]
