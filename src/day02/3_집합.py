...
print( s1 ) # {1, 2, 3}
s2 = set( "Hello" )
print( s2 ) # {'l', 'H', 'e', 'o'} , 실행 할때마다 순서는 랜덤으로 배치

#(2) 집합은 딕셔너리와 비슷 하므로 인덱싱이 없다, 리스트/튜플 로 타입 변환후 인덱싱 사용.
l1 = list( s1 )     # list( 요소 ) : 해당 요소를 리스트 타입 으로 반환 함수
print( l1 )     # [1, 2, 3]
t1 = tuple( s1 )    # list( 요소 ) : 해당 요소를 튜플 타입 으로 반환 함수
print( t1 )     # (1, 2, 3)
...
# 2. 합집합 , |(엔터위에원화기호를 shift+원화기호) , 1.집합1 | 집합2  , 2.집합1.union(집합2)
...
# s1.remove( 11 ) # KeyError: 11 , 집합내 존재하지 않는 요소 이므로 예외 발생